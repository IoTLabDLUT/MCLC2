function [u, a_gcv, cost] = mclc_in_with_gcv(x_sv, v_sv, u_sv, x_gcv, v_gcv, x_leader, v_leader, x_pv, v_pv, x_b, N, tb)
    A = tril(ones(N));
    if x_pv == -1
        x_pv = -1 * ones(N, 1);
    else
    ...
    for n = 1:N
        for i = 1:length(a)
            ...
            v_gcv = v_gcv + A * u_gcv;
            x_gcv = x_gcv + A * v_gcv;
            [u, us, ug] = mclc_in_cost_with_gcv(x_sv, v_sv, u_sv, x_leader, x_gcv, v_gcv, x_pv, x_b, n, a(i), N, tb);
            cost(n, i, :) = [u, us, ug];
        end
    end
...
function [u, us, ug] = mclc_in_cost_with_gcv(x0_sv, v0_sv, u0_sv, x_leader, x_gcv, v_gcv, x_pv, x_b, n, a, N, tb)
    vmax = 13.89;
    v_length = 5;
    dumax = 1;
    dumin = -1;
    ...
    % x <= x_b - tb * v FOR k < n
    Ab2 = (A + tb * E) * A;
    bb2 = x_b - x0_sv - (A + tb * E) * v0_sv;
    Ab2 = Ab2(1:n - 1, :);
    bb2 = bb2(1:n - 1, :);
    % x > x_b FOR k >= n
    if n <= N
        Ab3 = -A2(n, :);
        bb3 = x0_sv + A * v0_sv - x_b;
        bb3 = bb3(n, :);
    end
    % x <= x_leader for k < n
    if x_leader(1) == -1
    ...
    Aineq = [Av; Au; Ab2; Ab3; Apv; Agcv; Alv];
    bineq = [bv; bu; bb2; bb3; bpv; bgcv; blv];
