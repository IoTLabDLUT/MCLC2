function [u, cost] = mclc_in_without_gcv(x_sv, v_sv, u_sv, x_leader, v_leader, x_pv, v_pv, x_b, N, tb)
    umax = 2.5;
    vmax = 13.89;
    if x_pv == -1
    ...
    cost = zeros(N, 2);
    for n = 1:N
        [u, us] = mclc_in_cost_without_gcv(x_sv, v_sv, u_sv, x_leader, x_pv, x_b, n, N, tb);
        cost(n, :) = [u, us];
    end
...
function [u, us] = mclc_in_cost_without_gcv(x0_sv, v0_sv, u0_sv, x_leader, x_pv, x_b, n, N, tb)
    vmax = 13.89;
    v_length = 5;
    dumax = 1;
    dumin = -1;
    ...
    Aineq = [Av; Au; Ab2; Ab3; Apv; Alv];
    bineq = [bv; bu; bb2; bb3; bpv; blv];
    
%     Aineq * zeros(N, 1) - bineq
    lb = umin * ones(N, 1);
    ub = umax * ones(N, 1);
